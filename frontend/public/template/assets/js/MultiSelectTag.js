...
export function MultiSelectTag(el, customs = { shadow: false, rounded: true }) {
    ...
    function init() {
        ...
        input.addEventListener('keydown', (event) => {
            if (event.key === 'Backspace' && !event.target.value && inputContainer.childElementCount > 1) {
        ...
        window.addEventListener('click', (event) => {
            if (!customSelectContainer.contains(event.target)) {
                if ((event.target.nodeName !== "LI") && (event.target.getAttribute("class") !== "input_checkbox")) {
